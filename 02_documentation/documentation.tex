...
% after reading and understanding the examples listed in here.
%=============================================================================
...
% backslash (\). For instance to render text bold, we simply write
% \textbf{foo}, now 'foo' is going to shown in bold style.
...
%   - the fontsize (here "10pt")
%   - the paper size (here "a4paper")
%   - the pagetype (not present): 
%        - "twopage" different side margins for even and odd pages
%        - "oneside" same side margins for even and odd pages
%   - the number of columns (not present):
%       - twocolumn enables a twocolumn page layout
%       - onecolumn (generally the default) just uses one column
...
% you can define more code listing environments for other languages. For the 
...
% add a new page, this is optional
\newpage
...
% Here we add a new item to the description list
% Each item is added with a special command \item[ITEM_NAME] where ITEM_NAME
% is text you add.
% In this example ITEM_NAME is "\texttt{rm}". \texttt is a formatting command
...
% \LaTeX displays the latex logo, \xpsace a tiny space which vanished in front
% of a full stop.
...
% or "r" (right aligned). A vertical bar "|" stands for a vertical line in the
% outputted table:
\begin{tabular}{lll}
%                   each column is separated with an ampersand "&"
\verb$\textrm$		& \textrm{A normal text}\\ 
%                                     each new row is ended with "\\"
\verb$\textsl$ 		& \textsl{An italic text}\\
\verb$\texttt$ 		& \texttt{An monospace text}\\
% \verb$$ does the same as \texttt{} but also allows commands to be displayed
% in verbatim. The exclamation mark can be replaced with any other character.
\verb$\verb!!$ 		& \verb!An monospace text!
\verb$\textbf$ 		& \textbf{A bold text}\\
...
\verb$\underline$ 	& \underline{An underlined text} 
                      \uline{and here using the ulem-package}\\
...
\noindent In the simple math environment two different sized fractions
can be used; the small fractions $\frac{1}{2}$ or the normal sized
$\dfrac{1}{x}$.
...
Mathematical functions are  explicitly written in normal text not
math mode text to distinguish them from ordinary variables:
\[ \sin(x) = \text{sin}(x) \text{ {\bf and not }} sin(x)\]

\item[Varia]
\[ \left( \sqrt{\frac{A^C}{B_y}} +	 \sum_{i=1}^N a_i\right) \]
\[ A \stackrel {\lambda_a} {\longrightarrow} B \]
\[ \iint z\,dx \, dy \quad \mbox {\bf not} \quad \int\int z dx dy \]
				
{\Large
\[ \Leftarrow \  \Leftrightarrow \  \Longleftrightarrow \  \Rightarrow \_ 
    \Uparrow \  \Updownarrow \  \Downarrow	\]}
				
\[\bigcap \cap \sum \Pi \int_0^{2\pi} \]
