...
cami@bender:~/test$ ls
(to delete directories you have to add -r)
    \end{terminalcode}
...
\item[\texttt{mkdir}] make a new directory
    \begin{terminalcode}
[kurs05@manet Documents] ls
lessons
[kurs05@manet Documents] mkdir DIRECTORY
[kurs05@manet Documents] ls
DIRECTORY lessons

    \end{terminalcode}

\item[\texttt{mv}] move a directory into another directory
    \begin{terminalcode}
[kurs05@manet Documents] ls 
a b lessons
[kurs05@manet Documents] mv a b 
[kurs05@manet Documents] ls
b lessons
[kurs05@manet Documents] cd b 
[kurs05@manet b] ls
a
[kurs05@manet b] mv a .. (move directory a back in the directory Documents)
    \end{terminalcode}
...
[kurs05@manet Documents] ls
a b 
[kurs05@manet Documents] rename a c a  (rename FROM TO FILE) 

[kurs05@manet Documents] ls
b c
(If a filename already exists it will overwright it but only if it is empty)
    \end{terminalcode}
\newpage
\item[\texttt{cp}] copy a file and name it with a new name
    \begin{terminalcode}
[kurs05@manet Documents] ls
a
[kurs05@manet Documents] cp a b
[kurs05@manet Documents] ls
a b  
    \end{terminalcode}
...
\subsubsection{More Commands}
    \begin{tabular}{lll}
\verb$cat$      &  {give the text out}\\
\verb$cat-n$    &  {number all output lines}\\
\verb$clear$    &  {clear the display}\\
\verb$head$     &  {display first lines of a file}\\
\verb$kill$     &  {kill all running processes with a given file}\\
\verb$passwd$   &  {change your accounts password}\\
\verb$ping$     &  {see if a host is online and responds}\\
\verb$tail$     &  {displaz the last part of a file, mostly used for logs}\\
\verb$tar$      &  {used for un- and compressing a file}\\
\verb$tee$      &  {pipe fitting}\\
\verb$top$      &  {displaz and update sorted information about processes}\\
\verb$whoami$   &  {displaz effective user id}\\
    \end{tabular}
\subsection{SVN}
\subsubsection{Commands}
    \begin{terminalcode}
svn update up (update the file on the computer)
svn commit ci (update the file on the server)
svn checkout co (takes the file)
svn --help checkout (shows the help for checkout)
    \end{terminalcode}
\subsubsection{Svn commit in the terminal}
    \begin{enumerate}
\item{} \verb$ bash$
\item{} \verb$ export SVN_EDITOR=/usr/bin/vim$
\item{} \verb$ svn ci$
    \end{enumerate}
\subsubsection{Download the files from the svn-server}
    \begin{enumerate}
\item{} \verb$ search the files in the internet$
\item{} \verb$ copy the internetaddress$
\item{} \verb$ go back to the Terminal$
\item{} \verb$ write there: svn co INTERNETADDRESS (Enter)$
    \end{enumerate}
...
%=============================================================================
\subsection{Commands for beginning}
